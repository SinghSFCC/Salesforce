...
function updateSavedCards(args) {
  try {
    ...
    if (AdyenHelper.getAdyenRecurringPaymentsEnabled()) {
      var oneClickPaymentMethods = getOneClickPaymentMethods(customer); // To make it compatible with upgrade from older versions (<= 19.2.2), first delete payment instruments with METHOD_CREDIT_CARD
